...
def create_optimized_cnn_model(input_shape, num_classes=2, use_focal_loss=True):
    ...
    x = Conv1D(32, kernel_size=9, activation='relu', padding='same', kernel_regularizer=l2(0.001))(inputs)
    x = BatchNormalization()(x)
    x = MaxPooling1D(pool_size=1, padding='same')(x)
    
    # Second convolutional block with increased filters
    x = Conv1D(64, kernel_size=9, activation='relu', padding='same', kernel_regularizer=l2(0.001))(x)
    x = BatchNormalization()(x)
    x = MaxPooling1D(pool_size=1, padding='same')(x)
    
    # Third convolutional block with even more filters
    x = Conv1D(128, kernel_size=9, activation='relu', padding='same', kernel_regularizer=l2(0.001))(x)
    x = BatchNormalization()(x)
...
optimized_cnn_model = create_optimized_cnn_model(input_shape=(X_train.shape[1], X_train.shape[2]), use_focal_loss=True)
