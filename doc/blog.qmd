...
To better illustrate how the model's performance changes, the predicted biniding affinity column is dropped, and the model is retrained and evaluated. 
...
The accuracy only drops from 91% to 79%. However, this is misleading when considering the class imbalance of the data. The ratio of negative samples to positive is roughly 4:1, 
respectively. So, predicting the majority class — non-epitope — almost everytime would result in the majority of the testing data being correctly predicted and labeled.

A better performance metric to compare between models would be the model's recall rate on the positive class. 
How many of the epitopes in the testing data were correctly predicted and labeled? 
The same model, when predicted binding affinity was included as a predictor, produced a 74% recall rate while the current model has a much lower 18% recall rate. 
...
# --- Step 2: Sequence Encoding (No Padding Needed) ---
print(f"--- Step 2: Sequence Encoding ---")
all_sequences = combined_data['sequence'].values
unique_chars = sorted(list(set(''.join(all_sequences))))
print(f"Unique amino acids: {''.join(unique_chars)}")
...
X_onehot = np.zeros((len(int_sequences), 9, num_chars), dtype=np.float32)
...
print(f"X_onehot shape: {X_onehot.shape}") # Should be (num_samples, 9, num_chars)
...
print("--- Step 4: Model Definition ---")
# Model definition remains the same, it accepts input_shape parameter
...
# --- Step 5: Class Weights and Model Training ---
print("--- Step 5: Model Training ---")
# Calculate class weights (function definition is unchanged)
def compute_class_weights(y_train_labels):
    ...
    total = np.sum(counts)
    weights = {i: total / (len(counts) * count) for i, count in enumerate(counts) if count > 0}
    return weights

class_weights = compute_class_weights(y_train)
print(f"Calculated class weights: {class_weights}")

# Create the model with the correct input shape (length 9, num_chars features)
input_shape = (9, num_chars) # Use EXPECTED_LENGTH and updated num_chars
model = create_cnn_model(input_shape)
# print(model.summary()) # Optional: uncomment to see model layers
...
model_checkpoint = ModelCheckpoint('best_cnn_model_len9.keras', monitor='val_accuracy', save_best_only=True, verbose=0) # Changed filename slightly
...
# --- Step 6: Evaluation (Fixed Threshold 0.5) ---
print("--- Step 6: Evaluation ---")
# Optional: Load the best model saved by ModelCheckpoint
# model = tf.keras.models.load_model('best_cnn_model_len9.keras')

# Evaluate on test data (unchanged)
test_loss, test_accuracy = model.evaluate(X_test, y_test, verbose=0)
print(f"Test Loss: {test_loss:.4f}")
print(f"Test Accuracy (default 0.5 threshold): {test_accuracy:.4f}")

# Get prediction probabilities for the positive class (unchanged)
y_pred_proba = model.predict(X_test, verbose=0)
y_pred_proba_positive = y_pred_proba[:, 1]

# Apply fixed threshold of 0.5 (unchanged)
threshold = 0.5
y_pred = (y_pred_proba_positive >= threshold).astype(int)

# Print classification report (unchanged)
print("\nClassification Report (Threshold = 0.5):")
print(classification_report(y_test, y_pred))

# Calculate and plot confusion matrix (unchanged)
cm = confusion_matrix(y_test, y_pred)
...
# Plotting code remains the same as it uses the 'history' object
# and the final predictions/probabilities (y_test, y_pred_proba_positive)

# Plot training & validation accuracy/loss values
plt.figure(figsize=(12, 5))
plt.subplot(1, 2, 1)
# ... (accuracy plot code - unchanged)
...
# Plot Precision-Recall Curve (unchanged)
...
plt.figure(figsize=(7, 6))
# ... (PR plot code - unchanged)
plt.step(recall, precision, color='blue', where='post', label=f'PR curve (AP = {avg_precision:.2f})')
