...
To train a neural network, the sequence must be represented in a neumerical format. Assigning each amino acid a unique integer value up to 20, the sequences are converted to a list of integers. Then, the resulting integer sequences are one-hot encoded into a 3-dimensional arrary of shape (25628, 9, 20). 25,628 peptides, 9 amino acids in each peptide, 20 unique amino acids. The data is split into training, validation, and testing sets with a 70/15/15 ratio.
...
input_shape = (9, num_chars) # Use and updated num_chars
...
```{python}
# --- Class Weights and Model Training ---
...
The Convolutional Neural Network significantly outperforms the Random Forest classifier while relying only on the sequence data, and no additional features. The RF struggles immensely with identifying the actual epitopes (very low recall for Class 1), whereas the CNN, while still facing challenges with precision for Class 1, provides a much better balance and significantly higher recall for the positive class, making it the more effective model in this comparison.
