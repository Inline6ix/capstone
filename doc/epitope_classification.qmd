...
The following plots examine a single sample sequence, `DLSPDGPRS`, which the model correctly classified. The true label is Non-epitope, and the model predicted it as a Non-epitope with high confidence. The map shows which features drove this correct prediction.

```{python cnn_saliency_map}
#| label: fig-saliency-single
#| fig-cap: "Example Saliency Map for a Correctly Classified Sample (DLSPDGPRS)"
...
plt.title(f"Saliency Map for Sequence: {sample_sequence_str} (Predicted: {'Epitope' if predicted_class == 1 else 'Non-epitope'}) (True: {'Epitope' if sample_label == 1 else 'Non-epitope'})")
plt.tight_layout()
plt.show()

```

For this specific correctly classified sample (`DLSPDGPRS`):

*   The model's correct **Non-epitope** prediction was most strongly influenced by the **`G` at position 6** and the **`P` at position 7**.
*   The `L` at position 2 and the `P` at position 4 also showed moderate importance.

This suggests these positions, particularly the G and P towards the C-terminus, provided strong signals for the model to classify the sequence as a non-epitope.
